...
- **Properties (create/read):** `PropertyController`/`PropertyService` expose `GET /api/properties`, `GET /api/properties/{id}` and `POST /api/properties` for the same objects selectable when creating an appointment, including optional `latitude`/`longitude`; a created property always gets the default icon `pi-building` (no icon field in the create form), and update/delete are not yet implemented. Storage mirrors the appointment mechanism: `PropertyFileRepository` persists each property as its own JSON file under `remo.storage.properties.directory` (`ExampleObjects/`) — checked into the repository, unlike the gitignored `ExampleTerms/`, as seed data for a fresh checkout; using the "new property" feature locally now writes an additional file into this tracked folder, so check `git status` before committing.
...
- **Properties overview (create/read):** `PropertiesView` lists every property as a responsive card (icon, name, address, open/completed appointment counts, next appointment as a link into `AppointmentDetailDrawer`), pinned first by a `PropertyCreateCard` (large plus icon) that opens `PropertyFormDialog` (name, street/house number/address supplement as three separate fields recombined into a single stored `address` string, postal code, city, with a debounced Leaflet location preview via `PropertyLocationPreviewMap`, and client-side validation — street/city restricted to characters that occur in real German street/place names (letters/diacritics, digits, spaces, `. - ' /`), house number must be digits only and at most 5 digits, postal code exactly 5 digits, and duplicate detection independently flags a name or address already used by another property, and on submit a Nominatim-backed existence check blocks creation unless the address is a real `MATCH`, offering an acceptable one-click correction for a `SUGGESTION` (e.g. a wrong postal code) and a generic error for an unresolvable/ambiguous `NOT_FOUND` — each violation shown as a red-bordered field plus hint text, disabling submit); backed by `stores/properties.ts` and derived from `stores/appointments.ts`; the same store now backs the appointment form's "Objekt" dropdown (previously hardcoded example data). Editing/deleting properties is not yet implemented.
...
Binding Git workflow (remote, main branch, code owner, branch/PR process, issue effort labeling and matching model selection, and the mandatory post-implementation code review) is defined in [`.claude/rules/git-workflow.md`](.claude/rules/git-workflow.md).
