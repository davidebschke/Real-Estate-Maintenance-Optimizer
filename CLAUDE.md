...
- **Properties overview (create/read):** `PropertiesView` lists every property as a responsive card (icon, name, address, open/completed appointment counts, next appointment as a link into `AppointmentDetailDrawer`), pinned first by a `PropertyCreateCard` (large plus icon) that opens `PropertyFormDialog` (name, street/house number/address supplement as three separate fields recombined into a single stored `address` string, postal code, city, with a debounced Leaflet location preview via `PropertyLocationPreviewMap`, and client-side validation — house number must be digits only, postal code exactly 5 digits, duplicate detection independently flags a name or address already used by another property, and on submit a Nominatim-backed existence check blocks creation unless the address is a real `MATCH`, offering an acceptable one-click correction for a `SUGGESTION` (e.g. a wrong postal code) and a generic error for an unresolvable/ambiguous `NOT_FOUND` — each violation shown as a red-bordered field plus hint text, disabling submit); backed by `stores/properties.ts` and derived from `stores/appointments.ts`; the same store now backs the appointment form's "Objekt" dropdown (previously hardcoded example data). Editing/deleting properties is not yet implemented.
