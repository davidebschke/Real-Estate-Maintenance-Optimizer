...
- **`dto/`** – Data Transfer Objects fuer den Datenaustausch zwischen Backend und Frontend, entkoppelt von den internen Entities. Aktuell enthalten: `VersionResponse`, `ErrorResponse` sowie die Termin-DTOs (`CreateAppointmentRequest`, `MoveAppointmentRequest`, `AppointmentResponse` inkl. `actualEnd`/`completed`, `HistoryEntryResponse`).
...
- **`entity/`** – Domaenenmodell als Records. Aktuell enthalten: `Appointment` (inkl. `actualEnd`, `completed()` als abgeleitetem Zustand, Wither-Methoden `withSchedule`/`withActualEnd`), `HistoryEntry` und `HistoryEventType` (`CREATED`, `MOVED`, `COMPLETED`, `REOPENED`) – vorerst einfache Java-Records ohne JPA-/MongoDB-Mapping, da noch keine Datenbank existiert.
