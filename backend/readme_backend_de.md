...
- **`entity/`** – Domaenenmodell als Records. Aktuell enthalten: `Appointment` (inkl. `actualEnd`, `completed()` als abgeleitetem Zustand, Wither-Methoden `withSchedule`/`withActualEnd`), `HistoryEntry` und `HistoryEventType` (`CREATED`, `MOVED`, `COMPLETED`, `REOPENED`) sowie `Property` (`id`, `name`, `address`, `icon`) – vorerst einfache Java-Records ohne JPA-/MongoDB-Mapping, da noch keine Datenbank existiert.
...
### `src/test/java/...`
JUnit-Tests, Struktur spiegelt `src/main/java/...` wider. Deckt die Version-/Localization-/CORS-Konfiguration sowie die komplette Termin- und Objekt-Funktion ab (Repository, Service, Controller) – siehe `CLAUDE.md`, Abschnitt "Tests".
