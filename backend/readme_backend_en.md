...
- **`dto/`** – Data Transfer Objects for exchanging data between backend and frontend, decoupled from the internal entities. Currently contains: `VersionResponse`, `ErrorResponse`, and the appointment request/response DTOs (`CreateAppointmentRequest`, `MoveAppointmentRequest`, `AppointmentResponse` including `actualEnd`/`completed`, `HistoryEntryResponse`).
...
- **`entity/`** – Domain model records. Currently contains `Appointment` (including `actualEnd`, a derived `completed()` state, and wither methods `withSchedule`/`withActualEnd`), `HistoryEntry` and `HistoryEventType` (`CREATED`, `MOVED`, `COMPLETED`, `REOPENED`) — plain Java records for now, not JPA/MongoDB-mapped, since there is no database yet.
